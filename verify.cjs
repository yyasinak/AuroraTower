...
const storage=new Map();const els={};const noop=()=>{};const gradient={addColorStop:noop};
...
const sandbox={console,innerWidth:1280,innerHeight:800,devicePixelRatio:1,document:{getElementById:el,querySelectorAll:()=>[],addEventListener:noop},localStorage:{getItem:key=>storage.get(key)??null,setItem:(key,value)=>storage.set(key,String(value))},addEventListener:noop,requestAnimationFrame:noop,Math};
sandbox.window=sandbox;vm.createContext(sandbox);
const source=fs.readFileSync('game.js','utf8').replace('reset();function advanceFrame(dt)', 'globalThis.test={start,update,advanceFrame,draw,press,pause,reset,dash,buyBooster,earnCrystal,dancing,selectMap,bestKey,selectCharacter,chooseOutfit,currentSkin,activateRocket,theme,difficulty,registerLanding,die,release:code=>keys.delete(code),state:()=>({p,wallet,jumpBoost,map,tapes,crumbs,rocketTarget,spinTime,wardrobe,time,mode,platforms,highest,score,pollen,combo,comboTimer,bestCombo,lastFloor,meteors,skyTime,best,newRecord,hazard,selected,milestone,crystals,rush,orbs})};reset();function advanceFrame(dt)');
...
t.die();t.selectCharacter('ebucehil');assert.equal(t.currentSkin().name,'EbuCehil');assert(els.characters.innerHTML.includes('EbuCehil'));assert(els.wardrobe.classList.contains('hidden'));t.start();t.draw();t.update(1/120);assert.equal(t.state().crumbs.length,0);t.press('Space');for(let i=0;i<30;i++)t.update(1/120);assert(t.state().crumbs.length>0);assert(t.state().crumbs.length<=16);t.draw();let crumbLife=t.state().crumbs[0].life;t.pause();t.update(.5);assert.equal(t.state().crumbs[0].life,crumbLife);t.pause();t.start();assert.equal(t.state().crumbs.length,0);t.die();t.selectCharacter('white');t.start();t.press('Space');for(let i=0;i<30;i++)t.update(1/120);assert.equal(t.state().crumbs.length,0);
console.log('PASS: EbuCehil selection, drawing, airborne trail, particle cap, pause/reset and character isolation.');
t.die();t.selectMap('retro');assert.equal(t.state().map,'retro');assert(t.theme().name.includes('NEON 1986'));assert.equal(t.bestKey(),'auroraTowerBestRetro');assert.equal(t.state().mode,'menu');assert(!els.overlay.classList.contains('results-mode'));t.start();t.draw();assert(t.state().tapes.length>0);assert(t.state().platforms.some(a=>a.type==='boost'));t.selectMap('aurora');assert.equal(t.state().map,'retro');
s=t.state();s.tapes.push({x:s.p.x,y:s.p.y+18,taken:false});s.p.dash=false;t.update(1/120);assert.equal(t.state().score,150);assert(t.state().p.dash);
const pad=t.state().platforms.find(a=>a.type==='boost');Object.assign(t.state().p,{x:pad.x+pad.w/2,y:pad.y,ground:true,support:pad,vy:0});t.press('Space');t.update(1/120);assert(t.state().p.vy>750);t.draw();
for(const id of ['melduk','white']){t.die();t.selectCharacter(id);t.chooseOutfit(4);assert.equal(t.currentSkin().style,'retro');t.start();t.draw();}
t.die();t.selectMap('aurora');assert.equal(t.bestKey(),'auroraTowerBest');t.start();assert.equal(t.state().tapes.length,0);assert(!t.state().platforms.some(a=>a.type==='boost'));assert(t.theme().name.includes('GECE'));
console.log('PASS: selectable retro map, separate record key, map locks, tape reward/dash refill, neon jump pad, retro outfits, classic-map isolation.');
t.die();t.selectMap('retro');t.start();for(let i=0;i<70;i++)t.update(1/120);assert(t.dancing());t.draw();t.press('KeyD');t.update(1/120);assert(!t.dancing());t.pause();assert(!t.dancing());t.pause();t.die();t.selectMap('aurora');t.start();for(let i=0;i<70;i++)t.update(1/120);assert(!t.dancing());
t.die();t.state().wallet.balance=0;t.state().wallet.pending=[];assert(!t.buyBooster('rocket'));assert(!t.buyBooster('__proto__'));for(let i=0;i<42;i++)t.earnCrystal();assert.equal(t.state().wallet.balance,42);assert(t.buyBooster('magnet'));assert(!t.buyBooster('magnet'));assert(t.buyBooster('jump'));assert(t.buyBooster('rocket'));assert.equal(t.state().wallet.balance,0);assert.equal(JSON.parse(storage.get('auroraTowerWallet')).pending.length,3);
t.selectMap('retro');assert.equal(t.state().wallet.pending.length,3);t.start();assert.equal(t.state().wallet.pending.length,0);assert.equal(t.state().rush,15);assert.equal(t.state().jumpBoost,20);assert.equal(t.state().rocketTarget,860);assert.equal(JSON.parse(storage.get('auroraTowerWallet')).pending.length,0);t.pause();t.update(.5);assert.equal(t.state().jumpBoost,20);t.pause();t.start();assert.equal(t.state().rush,0);assert.equal(t.state().jumpBoost,0);assert.equal(t.state().rocketTarget,null);
t.earnCrystal();assert.equal(JSON.parse(storage.get('auroraTowerWallet')).balance,1);const reloadSandbox={...sandbox};vm.createContext(reloadSandbox);vm.runInContext(source,reloadSandbox);assert.equal(reloadSandbox.test.state().wallet.balance,1,'Wallet survives page reload');
console.log('PASS: retro idle dancing, motion/pause exclusions, wallet save/reload, insufficient funds, duplicate purchase guard, map carryover, one-use boosters and pause/reset.');
let player;class FakeAudio{constructor(){player=this;this.playing=false;}play(){this.playing=true;return Promise.resolve();}pause(){this.playing=false;}}
const musicSandbox={Audio:FakeAudio,document:sandbox.document,localStorage:sandbox.localStorage,URL:{createObjectURL:()=> 'blob:test',revokeObjectURL:noop}};musicSandbox.window=musicSandbox;vm.createContext(musicSandbox);vm.runInContext(fs.readFileSync('music.js','utf8'),musicSandbox);assert.equal(player.volume,.18);els.musicFile.onchange({target:{files:[{name:'test.mp3'}]}});assert(!player.playing);musicSandbox.TowerMusic.setPlaying(true);assert(player.playing);musicSandbox.TowerMusic.setPlaying(false);assert(!player.playing);musicSandbox.TowerMusic.setPlaying(true);els.musicToggle.onclick();assert(!player.playing);els.musicVolume.value='30';els.musicVolume.oninput();assert.equal(player.volume,.3);
console.log('PASS: local music selection, quiet default, play/pause, mute and independent volume.');
