...
const source=fs.readFileSync('game.js','utf8').replace('reset();function advanceFrame(dt)', 'globalThis.test={start,update,advanceFrame,draw,press,pause,reset,dash,selectMap,bestKey,selectCharacter,chooseOutfit,currentSkin,activateRocket,theme,difficulty,registerLanding,die,release:code=>keys.delete(code),state:()=>({p,map,tapes,crumbs,rocketTarget,spinTime,wardrobe,time,mode,platforms,highest,score,pollen,combo,comboTimer,bestCombo,lastFloor,meteors,skyTime,best,newRecord,hazard,selected,milestone,crystals,rush,orbs})};reset();function advanceFrame(dt)');
