...
public class MainActivity extends Activity implements SensorEventListener {
 private WebView web; private SensorManager sensors; private Sensor sensor; private long last,lastDispatch; private float lastX=Float.NaN;
 @Override public void onCreate(Bundle b){super.onCreate(b);
  ...
  web.setOverScrollMode(View.OVER_SCROLL_NEVER);web.setVerticalScrollBarEnabled(false);web.setHorizontalScrollBarEnabled(false);web.setBackgroundColor(0xff080c1c);web.setRendererPriorityPolicy(WebView.RENDERER_PRIORITY_IMPORTANT,true);
 ...
 @Override public void onResume(){super.onResume();last=0;lastDispatch=0;lastX=Float.NaN;if(web!=null)web.onResume();if(sensor!=null)sensors.registerListener(this,sensor,SensorManager.SENSOR_DELAY_GAME);}
 ...
 @Override public void onSensorChanged(SensorEvent e){if(e.timestamp-last<16000000L)return;last=e.timestamp;float x=-e.values[0];if(!Float.isNaN(lastX)&&Math.abs(x-lastX)<.025f&&e.timestamp-lastDispatch<80000000L)return;lastX=x;lastDispatch=e.timestamp;web.evaluateJavascript("window.TowerTilt&&window.TowerTilt.sample("+Float.toString(x)+")",null);}
