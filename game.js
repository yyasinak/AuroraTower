...
const mobileRendering=window.matchMedia?.('(pointer:coarse)')?.matches===true;
const renderQuality=mobileRendering?.68:1;
const STEP=mobileRendering?1/90:1/120;let accumulator=0;
...
let map='aurora',tapes=[],idleTime=0,jumpBoost=0,comboBoost=0,walletBoost=0,stormBrake=0,steelBoots=0;
const goods={magnet:{name:'Mıknatıs',price:25,detail:'İlk 15 sn mıknatıs + 2× kristal puanı'},jump:{name:'Yaylı ayakkabı',price:30,detail:'İlk 20 sn daha yüksek sıçrayış'},rocket:{name:'Roket kalkışı',price:50,detail:'Tura 10 katlık roketle başla'},combo:{name:'Kombo saati',price:35,detail:'İlk 30 sn kombo süresi 5 saniye'},wallet:{name:'Kristal çantası',price:45,detail:'İlk 25 sn kristaller cüzdana 2 kat eklenir'},storm:{name:'Fırtına freni',price:55,detail:'İlk 30 sn fırtına %35 daha yavaş'},steel:{name:'Çelik taban',price:42,detail:'İlk 30 sn kırılgan platformlar dayanır'}};
...
function earnCrystal(){wallet.balance=Math.min(Number.MAX_SAFE_INTEGER,wallet.balance+(walletBoost>0?2:1));saveWallet();setText('walletHud',mobileRendering?wallet.balance+' ◆':'CÜZDAN '+wallet.balance+' ◆');}
function useBoosters(){const pending=wallet.pending.slice();wallet.pending=[];saveWallet();for(const id of pending){if(id==='magnet')rush=15;if(id==='jump')jumpBoost=20;if(id==='rocket')activateRocket();if(id==='combo')comboBoost=30;if(id==='wallet')walletBoost=25;if(id==='storm')stormBrake=30;if(id==='steel')steelBoots=30;}renderShop();}
...
const characters={
 ...
 melduk:{enabled:false,name:'melduk',desc:'Bal poleni',body:'#542638',head:'#ecc09f',accent:'#f5c457',visor:'#765039',boots:'#302735',hair:'#594033',kind:'human',pollen:true},
...
try{const saved=localStorage.getItem('auroraTowerCharacter');if(Object.hasOwn(characters,saved)&&characters[saved].enabled!==false)selected=saved;}catch{}
function selectCharacter(id){if(!Object.hasOwn(characters,id)||characters[id].enabled===false||mode==='playing'||mode==='paused')return;selected=id;try{localStorage.setItem('auroraTowerCharacter',id);}catch{}renderCharacters();}
...
function resize(){W=innerWidth;H=innerHeight;const maxDpr=mobileRendering?1.55:2,pixelBudget=mobileRendering?1800000:3500000;DPR=Math.min(devicePixelRatio||1,maxDpr,Math.sqrt(pixelBudget/(W*H)));canvas.width=Math.round(W*DPR);canvas.height=Math.round(H*DPR);scale=Math.min(W/(mobileRendering?570:620),H/760,1.45);ox=(W-WORLD*scale)/2;}
...
function burst(x,y,color,n=12,power=150){n=Math.max(2,Math.ceil(n*renderQuality));for(let i=0;i<n;i++)particles.push({x,y,vx:rand(-power,power),vy:rand(-power,power),life:rand(.25,.65),max:.65,color,size:rand(2,5)});if(particles.length>180)particles.splice(0,particles.length-180);}
function generate(){
 ...
 for(;n*GAP<end;n++){
  ...
  const platform={n,y:n*GAP,x:position,prevX:position,base:x,w:width,moving,phase,moveSpeed:d.moveSpeed,amplitude:d.amplitude,breakDelay:d.breakDelay,type,broken:false,crack:0};platforms.push(platform);
  if(map==='retro'&&n%15===8)tapes.push({x:position+width/2,y:n*GAP+34,taken:false,anchor:platform});
  if(n>=35&&n%80===35&&Math.random()<.7)rockets.push({x:position+width/2,y:n*GAP+34,taken:false,anchor:platform});
  if(n>0&&n%3===0)orbs.push({x:position+width/2,y:n*GAP+34,taken:false,anchor:platform,loose:false});
 }
}

function reset(){time=0;accumulator=0;p={support:null,x:WORLD/2,y:0,vx:0,vy:0,w:24,h:36,ground:true,coyote:.1,wall:0,wallLock:0,dash:true,dashTime:0,face:1,land:0};platforms=[];orbs=[];particles=[];trails=[];camera=0;highest=0;score=0;maxFloor=0;combo=0;comboTimer=0;lastFloor=0;hazard=-460;elapsed=0;shake=0;jumpBuffer=0;keys.clear();idleTime=0;jumpBoost=0;comboBoost=0;walletBoost=0;stormBrake=0;steelBoots=0;tapes=[];crumbs=[];crumbClock=0;rockets=[];rocketTarget=null;rocketTrail=0;spinTime=0;pollen=[];pollenClock=0;bestCombo=0;landedFloor=0;newRecord=false;meteors=[];meteorClock=0;skyTime=0;milestone=0;crystals=0;rush=0;noticeTimer=0;$('notice').style.opacity=0;$('bottom').classList.remove('notice-active');$('combo').classList.remove('notice-active');$('combo').style.opacity=0;generate();p.support=platforms[0];}
function start(){reset();$('overlay').scrollTop=0;$('overlay').classList.remove('results-mode');mode='playing';window.TowerMusic?.setPlaying(map==='retro');useBoosters();$('overlay').classList.add('hidden');tone(550,.2);}
function overlay(kind){renderShop();const overlayNode=$('overlay');overlayNode.scrollTop=0;$('shop').classList.toggle('hidden',kind==='paused');
overlayNode.classList.remove('hidden');requestAnimationFrame(()=>{overlayNode.scrollTop=0;});
 ...
 if(kind==='paused'){
  ...
  setText('hint',mobileRendering?'DOKUN — yeniden başla':'ENTER — yeniden başla');
 }
}
...
window.TowerMobilePause=()=>{if(mode==='playing')pause();};
const jumpTouches=new Set();canvas.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'||mode!=='playing')return;e.preventDefault();canvas.setPointerCapture(e.pointerId);jumpTouches.add(e.pointerId);press('Space');});
function releaseTouch(e){jumpTouches.delete(e.pointerId);if(!jumpTouches.size)keys.delete('Space');}
canvas.addEventListener('pointerup',releaseTouch);canvas.addEventListener('pointercancel',releaseTouch);
const controlPointers=new Map(),controlCounts=new Map();
function holdControl(id,b){if(controlPointers.has(id))return;controlPointers.set(id,{key:b.dataset.key,button:b});controlCounts.set(b.dataset.key,(controlCounts.get(b.dataset.key)||0)+1);b.classList.add('pressed');press(b.dataset.key);}
function releaseControl(id){const item=controlPointers.get(id);if(!item)return;controlPointers.delete(id);const count=(controlCounts.get(item.key)||1)-1;if(count>0)controlCounts.set(item.key,count);else{controlCounts.delete(item.key);keys.delete(item.key);}if(![...controlPointers.values()].some(active=>active.button===item.button))item.button.classList.remove('pressed');}
function releaseChangedTouches(e){for(const touch of e.changedTouches||[])releaseControl('touch:'+touch.identifier);}
document.querySelectorAll('[data-key]').forEach(b=>{b.oncontextmenu=e=>e.preventDefault();if(mobileRendering){b.addEventListener('touchstart',e=>{e.preventDefault();for(const touch of e.changedTouches)holdControl('touch:'+touch.identifier,b);},{passive:false});b.addEventListener('touchend',releaseChangedTouches,{passive:false});b.addEventListener('touchcancel',releaseChangedTouches,{passive:false});}else{b.onpointerdown=e=>{e.preventDefault();holdControl('pointer:'+e.pointerId,b);try{b.setPointerCapture(e.pointerId);}catch{}};b.onpointerup=b.onpointercancel=e=>releaseControl('pointer:'+e.pointerId);}});
if(mobileRendering){document.addEventListener('touchend',releaseChangedTouches,{passive:false});document.addEventListener('touchcancel',releaseChangedTouches,{passive:false});}else{addEventListener('pointerup',e=>releaseControl('pointer:'+e.pointerId));addEventListener('pointercancel',e=>releaseControl('pointer:'+e.pointerId));}addEventListener('pointerup',releaseTouch);addEventListener('pointercancel',releaseTouch);addEventListener('blur',()=>{controlPointers.clear();controlCounts.clear();keys.clear();document.querySelectorAll('[data-key]').forEach(b=>b.classList.remove('pressed'));});
...
function syncAnchors(list){for(const item of list)if(item.anchor&&!item.loose&&!item.taken){item.x=item.anchor.x+item.anchor.w/2;item.y=item.anchor.y+34;}}
function updatePlatforms(dt){
 ...
 syncAnchors(orbs);syncAnchors(tapes);syncAnchors(rockets);
...
function update(dt){if(mode!=='playing'){if(mode==='menu'){time+=dt;camera=Math.sin(time*.15)*12;}return;}for(const key of controlCounts.keys())keys.add(key);time+=dt;elapsed+=dt;comboBoost=Math.max(0,comboBoost-dt);walletBoost=Math.max(0,walletBoost-dt);jumpBoost=Math.max(0,jumpBoost-dt);stormBrake=Math.max(0,stormBrake-dt);steelBoots=Math.max(0,steelBoots-dt);idleTime=p.ground&&Math.abs(p.vx)<10&&!keys.has('KeyA')&&!keys.has('KeyD')&&!keys.has('ArrowLeft')&&!keys.has('ArrowRight')?idleTime+dt:0;spinTime=Math.max(0,spinTime-dt);updatePlatforms(dt);updateSky(dt);rush=Math.max(0,rush-dt);noticeTimer=Math.max(0,noticeTimer-dt);$('notice').style.opacity=noticeTimer>0?1:0;$('bottom').classList.toggle('notice-active',noticeTimer>0);$('combo').classList.toggle('notice-active',noticeTimer>0);comboTimer=Math.max(0,comboTimer-dt);if(!comboTimer)combo=0;jumpBuffer-=dt;p.coyote-=dt;p.wallLock-=dt;p.land=Math.max(0,p.land-dt*4);p.dashTime=Math.max(0,p.dashTime-dt);let dir=(keys.has('ArrowRight')||keys.has('KeyD')?1:0)-(keys.has('ArrowLeft')||keys.has('KeyA')?1:0);const usingTilt=!dir&&!!window.TowerTilt?.axis();if(!dir)dir=window.TowerTilt?.axis()||0;if(dir)p.face=Math.sign(dir);
if(p.dashTime<=0&&p.wallLock<=0){if(dir)p.vx+=dir*(p.ground?1650:1080)*(usingTilt?(Math.sign(p.vx)!==Math.sign(dir)?2.6:1.45):1)*dt;else p.vx*=Math.exp(-(p.ground?8:1.3)*dt);p.vx=Math.max(-430,Math.min(430,p.vx));}
...
for(let i=platforms.length-1;i>=0;i--){const a=platforms[i];
 ...
 if(!wasGround||oldSupport!==a){
  ...
  if(a.type==='fragile'&&a.crack===0&&steelBoots<=0)a.crack=a.breakDelay;
 }
 break;
}

highest=Math.max(highest,p.y);maxFloor=Math.max(maxFloor,Math.floor(highest/GAP));
while(maxFloor>=milestone+25){milestone+=25;const century=milestone%100===0,reward=century?1000:250;score+=reward;announce(century?milestone+'. KAT':milestone+'. KAT!',century?theme().name.split(' / ')[1]+' · +'+reward+' puan · Zorluk '+(difficulty().tier+1):'+'+reward+' puan · Yükselmeye devam!');burst(p.x,p.y+40,century?'#72ffe4':'#ffe6a3',century?46:32,century?300:240);tone(century?1180:950,century?.42:.25,'triangle');}
setText('goal',mobileRendering?'HEDEF · '+(milestone+25)+'. KAT':'SONRAKİ HEDEF: '+(milestone+25)+'. KAT'+(comboBoost>0?' · KOMBO SAATİ '+Math.ceil(comboBoost)+' sn':'')+(walletBoost>0?' · 2× CÜZDAN '+Math.ceil(walletBoost)+' sn':''));setText('crystalStatus',mobileRendering?(rush>0?'ŞÖLEN · '+Math.ceil(rush)+' sn · 2×':'KRİSTAL · '+(crystals%5)+'/5'):(rush>0?'KRİSTAL ŞÖLENİ · '+Math.ceil(rush)+' sn · 2× puan + mıknatıs':'KRİSTAL '+(crystals%5)+'/5 · ŞÖLENE DOĞRU'));let target=Math.max(0,highest-H/scale*.46);camera+=(target-camera)*(1-Math.exp(-5*dt));const stormSpeed=elapsed<8?15:Math.min(65,27+elapsed*.3)+difficulty().stormBoost;hazard=Math.max(hazard+dt*stormSpeed*(stormBrake>0?.65:1),camera-170);generate();platforms=platforms.filter(a=>a.y>camera-550);orbs=orbs.filter(a=>a.y>camera-400);for(let o of orbs){if(o.taken)continue;let distance=Math.hypot(o.x-p.x,o.y-(p.y+18));if(rush>0&&distance<180){o.loose=true;o.x+=(p.x-o.x)*Math.min(1,dt*9);o.y+=(p.y+18-o.y)*Math.min(1,dt*9);distance=Math.hypot(o.x-p.x,o.y-(p.y+18));}if(distance<32){o.taken=true;score+=rush>0?50:25;crystals++;earnCrystal();if(crystals%5===0){rush=Math.max(rush,8);announce('KRİSTAL ŞÖLENİ','8 saniye mıknatıs + kristallerden 2× puan!');}tone(1200,.12);burst(o.x,o.y,'#ffe6a3',14,125);}}
...
updatePollen(dt);
if((Math.abs(p.vx)>200||p.dashTime>0)&&trails.length<28)trails.push({x:p.x,y:p.y+18,life:.22,color:p.dashTime>0?'#79ffe0':'#95beff'});trails.forEach(t=>t.life-=dt);trails=trails.filter(t=>t.life>0);particles.forEach(a=>{a.x+=a.vx*dt;a.y+=a.vy*dt;a.vy-=400*dt;a.life-=dt;});particles=particles.filter(a=>a.life>0);shake*=Math.exp(-12*dt);
...
function sy(y){return H-(mobileRendering?165:100)*scale-(y-camera)*scale;}
function drawFloorMarker(a,y){if(a.n<=0||a.n%5!==0)return;const checkpoint=a.n%50===0,century=a.n%100===0;ctx.save();ctx.textAlign='center';if(checkpoint){const color=century?'#70ffe3':'#ffd66f';ctx.shadowBlur=mobileRendering?8:18;ctx.shadowColor=color;round(a.x+a.w/2-37,y-24,74,18,6,century?'#123f48dd':'#44391bdd');ctx.shadowBlur=0;ctx.fillStyle=color;ctx.font='800 11px Segoe UI';ctx.fillText(a.n+'. KAT',a.x+a.w/2,y-11);}else{ctx.font='10px Segoe UI';ctx.fillStyle='#94b7cb';ctx.fillText(String(a.n).padStart(2,'0'),a.x+a.w/2,y+27);}ctx.restore();}
...
for(let i=0;i<(mobileRendering?3:4);i++){ctx.beginPath();ctx.moveTo(-100,H*.13+i*35);for(let x=-100;x<=W+100;x+=mobileRendering?55:35)ctx.lineTo(x,H*.16+i*30+Math.sin(x*.004+time*.12+i*.8)*65);ctx.lineTo(W+100,H*.6);ctx.lineTo(-100,H*.6);ctx.closePath();let g=ctx.createLinearGradient(0,H*.08,0,H*.6);g.addColorStop(0,['#4bdec012','#6ec9ff12','#a782f00d','#53efcf0b'][i]);g.addColorStop(1,'#00000000');ctx.fillStyle=g;ctx.fill();}
for(let i=0;i<(mobileRendering?58:100);i++){let x=((Math.sin(i*127.1)*.5+.5)*W),y=((i*137.7+camera*.09*scale)%(H+30));ctx.globalAlpha=.2+.4*(.5+.5*Math.sin(time+i));ctx.fillStyle='#d8f6ff';ctx.fillRect(x,y,i%9===0?2:1,i%9===0?2:1);}ctx.globalAlpha=1;
...
ctx.save();ctx.translate(ox+(Math.random()-.5)*shake,0);ctx.scale(scale,scale);const h=H/scale,Y=y=>H/scale-(mobileRendering?165:100)-y+camera;
...
for(let y=Math.floor(camera/130)*130-200;y<camera+h+150;y+=130){let yy=Y(y);ctx.strokeStyle='#91bbd90a';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(0,yy);ctx.lineTo(WORLD,yy);ctx.stroke();for(let x=0;x<WORLD;x+=90){let xx=x+(Math.round(y/130)%2)*45;ctx.beginPath();ctx.moveTo(xx,yy);ctx.lineTo(xx,yy-130);ctx.stroke();}}
...
for(let a of platforms){if(a.broken)continue;let y=Y(a.y);if(y< -40||y>h+30)continue;let color=a.type==='boost'?'#ff75d4':a.type==='spring'?'#f8cb7b':a.type==='fragile'?'#ed98b2':a.moving?'#a19cfa':'#6bdccb';ctx.shadowBlur=mobileRendering?0:12;ctx.shadowColor=color+'44';round(a.x,y,a.w,13,5,'#243d52');round(a.x,y,a.w,3,3,color);ctx.shadowBlur=0;round(a.x+7,y+13,a.w-14,5,2,'#091527');ctx.fillStyle='#99bac84a';ctx.fillRect(a.x+10,y+6,3,3);ctx.fillRect(a.x+a.w-13,y+6,3,3);drawFloorMarker(a,y);if(a.type==='boost'){ctx.fillStyle='#ffb4e9';ctx.font='bold 10px Segoe UI';ctx.textAlign='center';ctx.fillText('↑ ↑ ↑',a.x+a.w/2,y-7);}if(a.type==='spring'){ctx.strokeStyle=color;ctx.beginPath();ctx.moveTo(a.x+a.w/2-12,y-6);ctx.lineTo(a.x+a.w/2,y-12);ctx.lineTo(a.x+a.w/2+12,y-6);ctx.stroke();}if(a.crack>0){ctx.strokeStyle='#ffb0c2';ctx.beginPath();ctx.moveTo(a.x+a.w*.4,y);ctx.lineTo(a.x+a.w*.45,y+8);ctx.lineTo(a.x+a.w*.6,y+12);ctx.stroke();}}
