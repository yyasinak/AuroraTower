...
function update(dt){if(mode!=='playing'){if(mode==='menu'){time+=dt;camera=Math.sin(time*.15)*12;}return;}for(const key of controlCounts.keys())keys.add(key);time+=dt;elapsed+=dt;comboBoost=Math.max(0,comboBoost-dt);walletBoost=Math.max(0,walletBoost-dt);jumpBoost=Math.max(0,jumpBoost-dt);idleTime=p.ground&&Math.abs(p.vx)<10&&!keys.has('KeyA')&&!keys.has('KeyD')&&!keys.has('ArrowLeft')&&!keys.has('ArrowRight')?idleTime+dt:0;spinTime=Math.max(0,spinTime-dt);updatePlatforms(dt);updateSky(dt);rush=Math.max(0,rush-dt);noticeTimer=Math.max(0,noticeTimer-dt);$('notice').style.opacity=noticeTimer>0?1:0;$('bottom').classList.toggle('notice-active',noticeTimer>0);comboTimer=Math.max(0,comboTimer-dt);if(!comboTimer)combo=0;jumpBuffer-=dt;p.coyote-=dt;p.wallLock-=dt;p.land=Math.max(0,p.land-dt*4);p.dashTime=Math.max(0,p.dashTime-dt);let dir=(keys.has('ArrowRight')||keys.has('KeyD')?1:0)-(keys.has('ArrowLeft')||keys.has('KeyA')?1:0);const usingTilt=!dir&&!!window.TowerTilt?.axis();if(!dir)dir=window.TowerTilt?.axis()||0;if(dir)p.face=Math.sign(dir);
